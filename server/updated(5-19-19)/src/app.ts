...
function startServer(ordersDatastore: OrdersDatastore) {
  ...
  app.get('/users/username', async (request: Request, response: Response) => {
    const name = request.params.username;
    const user = await ordersDatastore.readUser(name);
    response.json({ user });
  });
  ...
  app.post('/posts', async (request, response) => {
    ...
    try {
      await ordersDatastore.createPost(title, content, author, likes, dislikes);
      response.sendStatus(201);
    } catch (error) {
      response.sendStatus(500);
    }
  ...
  app.patch('/posts/like/title', async (request, response) => {
    const title = request.params.title; 
    try {
      await ordersDatastore.likePostv2(title);
      response.sendStatus(200);
    } catch (error) {
      response.sendStatus(500);
    }
