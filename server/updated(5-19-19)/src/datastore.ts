...
export class OrdersDatastore {
  ...
  async createPost(title: string, content: string, author: string, likes: number, dislikes: number) {
    await this.posts.insertOne({ title, content, author, likes, dislikes });
  }
  ...
  async likePostv2(title: string) {
    await this.posts.update(
      {title: title}, //specifies document to be updated
      { $inc: { likes: 1 } } //increments number of likes for that document by 1
    );
  }
